...
def _retry_on_runtime_error(max_attempts=8, min_wait=2, max_wait=60, multiplier=2):
    """
    Minimal stand-in for tenacity's @retry(retry_if_exception_type(RuntimeError),
    wait_exponential(...)) — vendored so this module has no dependency on the
    `tenacity` package, whose version pinned by mega.py (<6.0.0) is incompatible
    with Python 3.11+ (it uses the long-removed `asyncio.coroutine`), which
    caused `import mega` to fail outright.

    Retries on RuntimeError with exponential backoff, capped at max_attempts
    so a persistent failure can't hang the calling thread forever.
    """
    def decorator(func):
